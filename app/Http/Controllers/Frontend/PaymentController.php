...
class PaymentController extends Controller
{
    ...
    public function paypalSuccess(Request $request)
    {
        ...
        if (!isset($response['status'])) {
            AlertService::error('Payment processing failed. Please try again.');
    ...
    {
        return view('frontend.pages.razopay-reditect');
    }

    public function razorpayPayment(Request $request): RedirectResponse
    {
        abort_if(empty($request->razorpay_payment_id), 404);

        OrderService::storeOrder(
            paymentId: $request->razorpay_payment_id,
            paidAmount: getPayableAmount() * config('settings.razorpay_rate'),
