...
class UserRoleController extends Controller implements HasMiddleware
{
    ...
    public function store(Request $request) : RedirectResponse
    {
        ...
        if ($role->name == 'Super Admin') {
            AlertService::error('Bạn không thể tạo Super Admin user.');
    ...
    public function update(Request $request, Admin $role_user)
    {
        if($role_user->hasRole('Super Admin')){
            AlertService::error('Bạn không có quyền để chỉnh');
            return to_route('admin.role-users.index');
        }
         $request->validate([
            'name' => ['required', 'string', 'max:255'],
    ...
    { 
        if($role_user->hasRole('Super Admin')){
            return response()->json(['status' => 'error', 'message' => 'Bạn không thể xóa']);
        }
        try{
            //remove role user
